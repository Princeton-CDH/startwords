...
function noteCloseLinkHandler(){if(location.hash=='#'+this.parentNode.parentNode.getAttribute('id')){event.preventDefault();pushHashAndFixTargetSelector('#');}
this.removeEventListener('click',noteCloseLinkHandler);return false;}
...
function pushHashAndFixTargetSelector(hash){history.pushState({},document.title,hash);const onpopstate=window.onpopstate;window.onpopstate=function(){window.onpopstate=onpopstate;history.forward();};history.back();}