...
While Hugo's default markdown handler — Goldmark — converts markdown tables to HTML tables, there's currently no way to add captions. This table shortcode takes a string of text and adds it to the `<caption>` tag for the table generated by the markdown.

Example input:
```
{{<table "Number of items sold per month.">}}
...
```
<table>
  <caption>Number of items sold per month.</caption>
  <thead>
    <tr>
      <th>month</th>
