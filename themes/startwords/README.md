...
- The feature essays for an issue are determined by order and the number of features in that issue (the features come first). By default, each issue has two features, but that can be customized by setting `num_features` in the issue metadata.  
