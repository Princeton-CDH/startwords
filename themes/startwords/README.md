...
You can use [hugo archetypes](https://gohugo.io/content-management/archetypes/#directory-based-archetypes) to quickly build new issues and articles. To build the fourth issue, for example, use this command at the project's top-level directory:
...
While Hugo's default markdown handler — Goldmark — converts markdown tables to HTML tables, there's currently no way to add captions. This table shortcode takes a string of text and adds it to the `<caption>` tag for the table generated by the markdown. The idea comes from [a post by Will Schenk](https://willschenk.com/articles/2020/styling_tables_with_hugo/).

Example input:
```
{{<table "Number of articles published per month.">}}
...
```
<table>
  <caption>Number of articles published per month.</caption>
  <thead>
    <tr>
      <th>month</th>
...
[view source](layouts/shortcodes/table.html)
