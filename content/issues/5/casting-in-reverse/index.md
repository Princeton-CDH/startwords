...
summary: In 1611, German ducal agent and art collector Philipp Hainhofer recorded two bronze statuettes in Markus Zeh’s imperial Augsburg collection.
...
And yet, it is not Giambologna's name that we find etched into them: instead, on the horse reads "ANT. SVSI/NI OPVS/FLORE," and the bull "ANT./SVSINI/F.", that is, on one, "the work of Antonio Susini, Florentine," on the other, "Antonio Susini made this."[^3]
...
The Princeton University Art Museum owns a gilt bronze corpus for a crucifix (also known as a Cristo Morto) originating from or around the workshop of Susini, one of many attributed to the artist. Ultimately, I would like to scan two of these and compare the 3D-models to determine where, if at all, molds were reused or altered. Though measurements can be taken manually from an object, it is inexact for curved surfaces in particular. Still in the early stages of this endeavor, I have been experimenting with the technologies available and the involved logistics of handling and imaging a sixteenth-century museum artifact. A series of test objects---some produced during my casting project---reveal the potentials and limits of doing so, and a preliminary photogrammetric model provides a standard on which to build.

I began first with a small brass frog and a tabletop scanner (Einscan SP). I expected a relatively simple result but, after my first few scans, it became clear that the process wouldn't be a straightforward one. Very little data was captured in these initial scans and only in areas where the patina was darker and matte.

{{< figure src="images/fig5-frog-fail.jpg" alt="Two views of sketchy digital textures that barely hint at the contours of a frog." caption="**Figure 5.** My initial --- unsuccessful --- scans of the brass frog." attr="Courtesy of author.">}}
{{< wrap class="txt-only" >}}
#-----------------------------------------------------------------------------------
...
The reflective surface of the metal was simply not captured by the scanner. For a project centered on bronze, this was a problem. In such cases, there is a matte, white spray used to cover objects so that they register more readily in the scanner's camera. This solution wasn't viable, however, when working with invaluable museum objects. There was a special irony to this hurdle. The characteristics that defined bronze---shiny, polished, reflective---made it resist reproduction, the very process for which it is indebted. What's more, in a project defined by its desire to recuperate a "hidden Susini," the fundamental work and gestures he was responsible for would be rendered, quite literally, invisible.

Naturally matte materials, like clay, or semi-gloss, like wax, on the other hand, translated well to a 3D-scan. I used both my wax model and fragments from my original clay model to test out the level of detail and structure it was able to pick up. Where the brass result was piecemeal at best, even from an initial scan of the clay the tabletop scanner was able to produce a far more cohesive and legible model.[^11] For the wax, which exceeded in height the limit of the tabletop scanner, I used a handheld version (Einstar).

{{< figure src="images/fig6-drapery-scan.jpg" alt="Two views of a digital scan of a robed figure. The scan on the right is of much higher quality, showing some of the statue's color and texture." caption="**Figure 6.** (Left) 3D-scan of terracotta clay drapery fragment (Einscan SP, tabletop). (Right) 3D-scan of wax model (Einstar, handheld)." attr="Courtesy of author.">}}
{{< wrap class="txt-only" >}}
#-----------------------------------------------------------------------------------
...
|
| CAPTION: **Figure 6.** (Left) 3D-scan of terracotta clay drapery fragment (Einscan SP, tabletop). (Right) 3D-scan of wax model (Einstar, handheld).
|
| ATTRIBUTION: Courtesy of author.
#-----------------------------------------------------------------------------------
{{</ wrap >}}

In both, the technology shined and produced the kinds of results I was hoping for; the only problem was that those results concerned the wrong materials. If the problem then wasn't the technology, how best could I mitigate the other factors that may be contributing to the quality of the scan? If the object's reflectivity was the dominant issue, that is, how do we mitigate its refraction? One option was to limit the light source. I closed the shades and turned off all of the overhead lights. The scan was only marginally better. At this point I realized that it was not only the position and strength of the light source that mattered, but also the orientation of the object. Instead of positioning the frog flat on its case, I tried two orientations on its side, horizontal and vertical, securing it with a bit of casting wax (something with enough tensile strength to keep the frog vertical but not enough to obscure the object itself). These changes garnered shockingly better results, and each orientation captured slightly different data. Not only did the scans now capture the contours of the object in full but, when multiple scans were aggregated, they even captured and reproduced the reflective surface of the brass itself.[^12]

{{< figure src="images/fig7-frog-combined.jpg" alt="Three views of a patchy digital scan of a small metal frog statue from different angles. The view on the right clearly reads 'CANADA' printed across the bototm." caption="**Figure 7.** (Left) Illustration of showing number of scans and variety of orientation that were aggregated. (Center) Combined scan of brass frog from the top. (Right) Combined scan of brass frog from the bottom.">}}
{{< wrap class="txt-only" >}}
#-----------------------------------------------------------------------------------
...
|
| CAPTION: **Figure 7.** (Left) Illustration of showing number of scans and variety of orientation that were aggregated. (Center) Combined scan of brass frog from the top. (Right) Combined scan of brass frog from the bottom.
|
| ATTRIBUTION: Courtesy of author.
#-----------------------------------------------------------------------------------
{{</ wrap >}}

In 3D-scanning, one needs to "mesh" their model, which effectively fills in the data gaps and together renders a more life-like scan.[^13] If the raw scans can be seen as a hollow, untreated bronze cast---with imperfections, sprue holes, and other proverbial "casting flaws"---the mesh function becomes surrogate for the finishing processes of polish and repair. The results of the mesh were striking, if also unexpected.

{{<sketchfab id="69fdd11c63da4927a5d7d07bf643fcda" alt="Still from a 3D model of a bronze frog, viewed head on." pdf-img="images/fig8-sketchfab-frog-still.png" pdf-alt="Still image of a gilt frog, captured from a 3D scan of the object." caption="**Figure 8.** 3D-scan of brass frog, using Einscan SP, tabletop. The black shapes on the bottom and side capture the casting wax used to support the object.">}}
{{< wrap class="txt-only" >}}
⩩-----------------------------------------------------------------------------------⟩
| FIGURE 8. 3D-scan of brass frog, using Einscan SP, tabletop. The black shapes on the bottom and side capture the casting wax used to support the object. Image courtesy of author.
⩩-----------------------------------------------------------------------------------⟩
{{</ wrap >}}

On the one hand, the surface is surprisingly reflective, reading without doubt as metallic material. On the other, however, it is dark and the details blurry, two characteristics that didn't come to define the aggregative scan from which we started. Where both finishing stages in bronze and 3D-scanning brought out the polish, in bronze the details are also defined; here, in pixels, they are obfuscated. Post-processing in EinScan and Rhino did little to remedy these effects, and more experimentation here is needed. Either way, it would seem that the reflectivity of the material not only interfered with the laser scan itself, but also the post-processing, only in the inverse---where the laser scan was at first unable to pick up highlights, the mesh processing gave all its attention to the reflective, the negating, in turn, all other surface textures.

In addition to 3D-scanning, another technology that offers a similar result is photogrammetry. Where 3D-scanning affords more accuracy in shape and scale, photogrammetry offers better results in color and surface texture ---3D-scanning uses laser measurements to build the model while the latter creates its model using photographs. To test the potential of this technology, I used MetaScan Pro to create a model of a bronze that I had just recently cast and removed from its investment. The cast had yet to be fully cleaned and polished and as such displayed a number of surfaces, from rough to polished bronze: perfect for testing photogrammetry's ability to handle a variety of surfaces, reflective or otherwise. I made a model using 200 photographs of the cast. Though there are some losses and peculiarities, the result captures the surface in relatively high detail and documents the dimensions with some accuracy. (It was about half an inch off in all directions, which could have been due to a skewed angle of the camera, for example.) The two polished areas on the back of the cast (where the vents had been cut) are picked up without difficulty and read in equal detail to their rougher counterparts. A three-part comparison between stills from the photogrammetric, the 3D-scanned model (using the handheld Einstar), and a photograph show some of the differences between technologies: most notably here, the 3D-scan is unfocused, and the surface textures read more or less homogenous; the photogrammetric still, on the other hand, perhaps expectedly, reads closer to that of the still photograph.

{{< figure src="images/fig9-cast-w-scan.jpg" alt="Three views of a slab of material: a photograph of it held in the hand, a photogrammetry image of the slab in high resolution, and a 3D scan of the object with slightly less resolution." caption="**Figure 9.** (Left) Still from 3D-scan, Einstar handheld. (Center) Still from photogrammetry model. (Right) Still photo of cast.">}}
{{< wrap class="txt-only" >}}
#-----------------------------------------------------------------------------------
...
[^2]: Filippo Baldinucci, "Notizie di Giovanni Bologna," in *Notizie De\' Professori Del Disegno Da Cimabue: In Qua, Per Le Quali Si Dimostra Come, e Per Chi Le Bell\' Arti Di Pittura, Scultura, e Architettura Lasciata La Rozzezza Delle Maniere Greca, e Gottica, Si Siano In Questi Secoli Ridotte All\' Antica Loro Perfezione,* vol. 2 (Firenze: 1846), 583.

[^3]: The inscriptions, as quoted in text, relate to the Roman casts. There is slight variation in the Louvre and DIA versions: the Louvre inscription reads" "ANT°-/SVSI/NI.F" and the DIA "ANTo. SVSINI \| FLORE. OPVS.

[^4]: For a comprehensive account of these discussions, see the Peggy Fogelman's catalogue entry for the Getty's (unsigned) pair. Peggy Fogelman, "Lion Attacking a Horse and Lion Attacking a Bull," In *Italian and Spanish Sculpture: Catalogue of the J. Paul Getty Museum Collection*, edited by Peggy Fogelman, Peter Fusco, and Marietta Cambareri (Los Angeles: J. Paul Getty Museum, 2002), 177-189.

[^5]: James Holderbaum, *The Sculptor Giovanni Bologna* (New York: Garland Pub., 1983), 255; Katherine Watson, *Pietro Tacca, successor to Giovanni Bologna* (New York: Garland Pub., 1983), 47.

[^6]: In a letter from from Giambologna to Belisario Vinta, August 6, 1605. Cited in Watson, *Pietro Tacca*, 33, fn. 37.

[^7]: For these concepts, see Steven Shapin, "The Invisible Technician," *American Scientist* 77 (1989): 554-63; Marjorie Perloff, *Unoriginal Genius: Poetry by Other Means in the New Century* (Chicago: University of Chicago Press, 2010); Jacques de Caso, "Serial Sculpture in Nineteenth-Century France," in *Metamorphoses in Nineteenth-Century Sculpture*, ed. Jeanne L. Wasserman (Cambridge: Fogg Museum, 1975), 1-28.

[^8]: It should be acknowledged, of course, that signatures, especially those in bronze, do not in and of themselves assert authorship with absolute certainty. For our purposes, however, they offer measures on which to work.

[^9]: Vannoccio Biringuccio, *Pirotechnia* \[1540\], trans. Cyril Stanley Smith and Maratha Teach Gnudi (Cambridge: M.I.T. Press, 1966), 220.

[^10]: I am also inspired here by other sculpture 3D-modelling initiatives in art history, such as the collaborative multidisciplinary project, "The Technical Study of Bernini's Bronzes," between the University of Toronto, the Art Gallery of Ontario, and the Getty Research Institute.
...
[^15]: I took the photogrammetry model's measurements using MetaScan. While the proportions were similar, MetaScan is peculiarly measuring at 10x magnification, labeling the object's 5cm forearm, for example, as 50.

[^16]: Michael Cole, "Bronze as Model," *West 86^th^: A Journal of Decorative Arts, Design History, and Material Culture,* Vol. 28, No. 2 (Fall/Winter, 2021): 232-239.
